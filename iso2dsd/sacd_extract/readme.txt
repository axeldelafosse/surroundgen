...
**************************************************************
Here are some example on how to use the app in a batch script

*************************************************************

Linux:

a) Simple scaning script for corrupted iso's:

#! /bin/sh
#

echo "Starting batch sacd_extract!!\n"
find '/your folder with many iso' -name '*.iso' -type f -exec echo "Start processing ISO file:" {} \; -exec ./sacd_extract -i {} \; | tee batch-sacd-extract-log.txt

b) More complex run (e.g. extracting dsf):

#! /bin/sh
#

echo "Starting batch sacd_extract!!\n"
find '/your folder with many iso' -name '*.iso' -type f -exec echo "Start processing ISO file:" {} \; -exec ./sacd_extract -s -2 -m -i {} -o '/home/user/output folder' \;
...
for /F "usebackq delims==" %%I in (`dir /B /S "X:\your folder\*.iso"`) do "X:\app folder\sacd_extract" -i "%%I" -o "X:\output folder DSF" done >> "log.txt"

b) More complex run (e.g. extracting dsf):

for /F "usebackq delims==" %%I in (`dir /B /S "X:\your folder\*.iso"`) do "X:\app folder\sacd_extract" -P -C -s -2 -m -i "%%I" -o "X:\output folder DSF" done
